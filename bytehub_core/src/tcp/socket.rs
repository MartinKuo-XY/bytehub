...
use bytehub_syscall::socket2::Socket;
...
fn new_socket(addr: &SocketAddr, mptcp: bool) -> Result<Socket> {
    ...
    {
        let call = if mptcp {
            bytehub_syscall::new_mptcp_socket
        } else {
            bytehub_syscall::new_tcp_socket
        };
        call(addr)
    }
    ...
    {
        bytehub_syscall::new_tcp_socket(addr)
    }
}
...
pub fn bind(laddr: &SocketAddr, bind_opts: BindOpts) -> Result<TcpListener> {
    ...
    if let Some(iface) = bind_interface {
        bytehub_syscall::bind_to_device(&socket, &iface)?;
    }
...
pub(super) mod keepalive {
    use super::*;
    pub use bytehub_syscall::socket2::{SockRef, TcpKeepalive};

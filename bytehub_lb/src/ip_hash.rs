...
/// Per-backend health state for iphash failover.
#[derive(Debug)]
...
/// Iphash balancer with optional health-check failover.
#[derive(Debug)]
pub struct IpHash {
    nodes: Vec<Node>,
...
impl Balance for IpHash {
    ...
    fn next(&self, state: &Self::State) -> Option<Token> {
        ...
        let start = match self.nodes.binary_search_by_key(&hash, |node| node.hash) {
            Ok(idx) => idx,
            Err(idx) if idx >= self.nodes.len() as usize => 0,
            Err(idx) => idx,
        };

        // Ring probing: walk clockwise until a healthy node (or all tried).
        let now = now_secs();
        let len = self.nodes.len();
        for offset in 0..len {
            let idx = (start + offset) % len;
            let token_idx = self.nodes[idx].token.0 as usize;

            if let Some(cfg) = &self.config {
                let backend = &self.backends[token_idx];
                let fails = backend.fails.load(Ordering::Relaxed);

                if fails >= cfg.max_fails {
                    let checked = backend.checked.load(Ordering::Relaxed);
                    if now < checked {
                        // Unhealthy, try next.
                        continue;
                    }
                    // Timeout expired, auto-recover.
                    backend.fails.store(0, Ordering::Relaxed);
                    backend.checked.store(0, Ordering::Relaxed);
                }
            }

            return Some(self.nodes[idx].token);
        }

        // All backends unhealthy: fall back to first token.
        if self.config.is_some() {
            return Some(self.nodes[start].token);
        }

        Some(self.nodes[start].token)
    }
    ...
    fn on_failure(&self, token: Token) {
        ...
        let idx = token.0 as usize;
        if let Some(backend) = self.backends.get(idx) {
            let fails = backend.fails.fetch_add(1, Ordering::Relaxed) + 1;
            if fails >= cfg.max_fails {
                let now = now_secs();
                backend
                    .checked
...
mod tests {
    ...
    fn ih_any_hash() {
        ...
        run!(b"bytehub", 885396906);
        run!(b"1 bytehub", 4115282535);
        run!(b"2 bytehub", 1326782105);
        run!(b"3 bytehub", 1796078392);
        run!(b"10 bytehub", 2265248424);
        run!(b"100 bytehub", 4289654351);
    }

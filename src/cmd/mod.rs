...
use bytehub_core as _core;
use _core::bytehub_io as _io;
use _core::bytehub_syscall as _syscall;
...
pub fn scan() -> CmdInput {
    ...
    let app = Command::new("bytehub").about("A high efficiency relay tool").version(ver);
    ...
    let app = app
        ...
        .override_usage("bytehub [FLAGS] [OPTIONS]");
...
fn handle_matches(matches: ArgMatches) -> CmdInput {
    ...
    if matches.get_flag("daemon") {
        // 修改为 _syscall
        _syscall::daemonize("bytehub is running in the background");
    }
